...
pass/fail summary to Slack for manual confirmation. Read-only reporting — it
never acknowledges or writes anything back to Spring Systems.

Two ways to run a review:
- **Batch, via CLI** (`main.py`) — pulls all new POs for the retailer and posts
  a summary for each.
- **On demand, via Slack** (`slack_listener.py`) — type `/po-review <po_num>`
  in Slack to check one specific PO.
...
| `SLACK_BOT_TOKEN` / `SLACK_APP_TOKEN` | `slack_listener.py` | Bot (`xoxb-`) and app-level (`xapp-`) tokens for the `/po-review` Socket Mode listener. See below. |

The SKU join key is `product.product_vendor_item_num` (our own SKU) — **not**
`po_item_buyer_item_num` (the retailer's internal item number).
...
## Setting up the `/po-review` Slack slash command
...
4. **Slash Commands** → create `/po-review`. The Request URL field can be
   left blank/placeholder — Socket Mode doesn't call it.
5. Invite the bot to the channel where you want to use `/po-review`.
6. Run it:
   ```bash
...
The listener does **not** touch `processed_pos.json` — it's an ad hoc,
one-off check, independent of the batch CLI's dedup state.
...
**Caveat:** this only keeps `/po-review` alive while this machine is powered
on, awake, and WSL is running — fine for a single-person MVP, but not a
reliable deployment for a team to depend on. If this proves useful, move
...
| `slack_listener.py` | `/po-review` Socket Mode listener. |
...
| `slack_notify.py` | Formats a `POResult` into Slack message text and posts it via webhook. |
